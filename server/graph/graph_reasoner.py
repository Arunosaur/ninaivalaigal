...
class GraphReasoner:
    ...
    async def explain_context(
        ...
        await self.redis_client.setex(
            cache_key,
            self.cache_ttl,
            json.dumps(explanation.__dict__, default=str)
        )
    ...
    async def infer_relevance(
        ...
        suggested_agents = self._select_top_suggestions(
            agent_scores, min(suggestion_count, 3)  # Fewer agent suggestions
        )
        ...
        await self.redis_client.setex(
            cache_key,
            self.cache_ttl,
            json.dumps(inference.__dict__, default=str)
        )
    ...
    async def _find_reasoning_paths(
        ...
        LIMIT 10
        """ % (max_depth, start_node_id, target_node_id)
    ...
    def _select_primary_path(self, paths: List[ReasoningPath]) -> Optional[ReasoningPath]:
        ...
        scored_paths = [
            (path, path.total_weight * path.confidence) 
            for path in paths
        ]
    ...
    async def _calculate_path_relevance(self, paths: List[ReasoningPath]) -> float:
        ...
        total_relevance = sum(
            path.total_weight * path.confidence 
            for path in paths
        )
    ...
    async def _gather_supporting_evidence(
        ...
        for record in result:
            evidence.append(
                f"Recent {record['rel_type']} with {record['related.id']}"
            )
    ...
    async def _score_memory_suggestions(
        ...
        for memory_id in memory_ids:
            ...
            if result:
                ...
                final_score = (proximity_score * 0.4 + 
                              base_score * 0.4 + 
    ...
    async def _generate_network_insights(
        ...
        if hubs:
            top_hub = hubs[0]
            insights.append(f"Memory hub '{top_hub['node_id']}' has {top_hub['connections']} connections")
