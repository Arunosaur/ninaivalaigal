...
from rbac_middleware import require_permission, require_role, get_rbac_context
from rbac.permissions import Role, Action, Resource
from security_integration import security_manager
from security.audit import security_alert_manager, AlertSeverity, SecurityEventType
...
async def get_redaction_audit_logs(
    ...
    tier: Optional[str] = Query(None, description="Filter by sensitivity tier")
):
...
async def test_redaction(
    request: Request,
    test_text: str,
    tier: Optional[str] = Query("internal", description="Sensitivity tier to test"),
    current_user: User = Depends(get_current_user)
):
