...
async def add_comment(
    ...
    new_comment = {
        ...
        "metadata": {"comment_type": comment_type, "edited": False, "reactions": {}},
    }
...
async def get_discussion_stats(
    ...
    stats = {
        ...
        "memory_engagement": {},
        "approval_engagement": {},
        "thread_depth_analysis": {"top_level": 0, "replies": 0, "max_depth": 0},
    }
    ...
    return {
        ...
        "engagement_insights": {
            ...
            "most_active_user": (
                max(
                    stats["user_engagement"].items(),
                    key=lambda x: x[1]["comment_count"],
                )
                if stats["user_engagement"]
                else None
            ),
            "dominant_sentiment": (
                max(stats["sentiment_distribution"].items(), key=lambda x: x[1])
