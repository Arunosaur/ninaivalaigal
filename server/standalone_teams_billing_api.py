...
# Initialize router
router = APIRouter(prefix="/standalone-teams", tags=["billing"])

# Stripe configuration
stripe.api_key = os.getenv("STRIPE_SECRET_KEY", "sk_test_...")

# Mock data stores for demo (use actual database in production)
discount_codes_store: dict = {}
team_credits_store: dict = {}
nonprofit_applications_store: dict = {}
team_usage_stats_store: dict = {}
billing_plans = {
    "free": {
        "name": "Free Plan",
        "price": 0.00,
...
async def get_team_billing_dashboard(
    ...
    plan_limits: Dict[str, int] = current_plan["features"]  # type: ignore
...
async def upgrade_team_billing_plan(
    ...
    plan_details = billing_plans.get(new_plan, billing_plans["free"])
    amount: float = float(plan_details["price"])  # type: ignore
    ...
    # Apply credits
    final_amount: float = deduct_team_credits(team_id, amount)
