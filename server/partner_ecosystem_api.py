...
async def calculate_partner_revenue_share(
    ...
    period_conversions = [
        ...
        if (
            ...
            and r["status"] == ReferralStatus.CONVERTED
            and r.get("converted_at")
            and start_date <= datetime.fromisoformat(r["converted_at"]) <= end_date
        )
    ]
    ...
    return RevenueShareCalculation(
        partner_id=partner_id,
        calculation_period={"start_date": start_date, "end_date": end_date},
