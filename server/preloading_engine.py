...
from relevance_engine import get_relevance_engine, RelevanceEngine
import structlog
...
class MemoryPreloadingEngine:
    ...
    async def get_recent_memories(self, user_id: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("Retrieved recent memories for preloading", 
                        user_id=user_id, 
    ...
    async def get_frequent_memories(self, user_id: str, limit: int = 30) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("Retrieved frequent memories for preloading",
                        user_id=user_id,
    ...
    async def get_important_memories(self, user_id: str, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("Retrieved important memories for preloading",
                        user_id=user_id,
    ...
    async def select_memories_for_preloading(self, user_id: str) -> Dict[str, List[Dict[str, Any]]]:
        ...
        try:
            ...
            logger.info("Selected memories for preloading",
                       user_id=user_id,
    ...
    async def preload_memories_to_cache(self, user_id: str, memories_by_strategy: Dict[str, List[Dict[str, Any]]]) -> Dict[str, int]:
        ...
        try:
            ...
            for strategy, memories in memories_by_strategy.items():
                ...
                logger.debug("Preloaded memories to cache",
                           user_id=user_id,
                           strategy=strategy,
                           count=len(memories),
                           ttl=ttl)
    ...
    async def get_preloaded_memory(self, user_id: str, memory_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if cached_memory:
                logger.debug("Preloaded memory cache hit", 
                           user_id=user_id, 
                           memory_id=memory_id)
            ...
            logger.error("Error getting preloaded memory", 
                        user_id=user_id, 
                        memory_id=memory_id, 
                        error=str(e))
            return None
    ...
    async def background_cache_warming(self, user_ids: List[str] = None):
        ...
        try:
            ...
            logger.info("Starting background cache warming", user_count=len(user_ids) if user_ids else "all")
...
async def get_preloaded_memory(user_id: str, memory_id: str) -> Optional[Dict[str, Any]]:
    """Get a preloaded memory from cache"""
    engine = await get_preloading_engine()
