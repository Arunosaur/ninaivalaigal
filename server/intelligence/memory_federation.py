...
from .models import (
    ...
    SharingPolicy, PrivacyLevel, FederationMetrics
)
...
class MemoryFederationEngine:
    ...
    async def federate_memories(
        ...
        try:
            ...
            target_contexts = {
                team: await self._get_team_context(team) 
    ...
    async def _apply_privacy_filters(
        ...
        if privacy_level == PrivacyLevel.CONFIDENTIAL:
            ...
            sensitive_fields = ['personal_data', 'credentials', 'api_keys', 'internal_urls']
    ...
    def _update_federation_metrics(self, successful: int, filtered: int, processing_time: float):
        ...
        self.metrics.federation_latency_ms = (
            ...
            (1 - alpha) * self.metrics.federation_latency_ms
        )
