                ...
                processing_result.update(
                    {
                        ...
                        "invoice_id": invoice.get("id"),
                        "actions_taken": [
                            "recorded_payment_failure",
                            "initiated_retry_sequence",
                        ],
                    }
                )
...
async def create_stripe_customer(
    ...
    try:
        ...
        stripe_customer = stripe.Customer.create(
            email=request.email,
            name=request.name,
            metadata={"team_id": request.team_id, "platform": "ninaivalaigal"},
...
async def create_stripe_subscription(
    ...
    try:
        ...
        subscription_data = {
            ...
            "metadata": {"team_id": request.team_id, "plan_id": plan_id},
        }
        ...
        if request.trial_days:
            subscription_data["trial_period_days"] = request.trial_days  # type: ignore
...
async def generate_invoice(
    ...
    invoice_count = len(billing_invoices_store) + 1
    invoice_number = f"INV-{timestamp}-{invoice_count:04d}"
