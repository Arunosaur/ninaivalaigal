    ...
    def _initialize_metrics(self):
        ...
        core_metrics = [
            ...
            (
                ...
                MetricType.COUNTER,
                "Total number of redaction failures",
            ),
            (
                "redaction_duration_seconds",
                MetricType.HISTOGRAM,
                "Time spent on redaction operations",
            ),
            # Idempotency metrics
            (
            ...
            (
                ...
                MetricType.COUNTER,
                "Total fail-closed policy triggers",
            ),
            (
            ...
            (
                "tier_policy_enforcements_total",
                MetricType.COUNTER,
                "Total tier policy enforcements",
            ),
            # RBAC metrics
            (
                "rbac_access_granted_total",
                MetricType.COUNTER,
                "Total RBAC access grants",
            ),
            (
            ...
            (
                "rbac_token_verifications_total",
                MetricType.COUNTER,
                "Total JWT token verifications",
            ),
            # Multipart metrics
            (
                "multipart_parts_processed_total",
                MetricType.COUNTER,
                "Total multipart parts processed",
            ),
            (
                "multipart_parts_total",
                MetricType.COUNTER,
                "Total multipart parts received",
            ),
            (
                "multipart_bytes_total",
                MetricType.COUNTER,
                "Total multipart bytes processed",
            ),
            (
                "multipart_reject_total",
                MetricType.COUNTER,
                "Total multipart rejections with reason",
            ),
            (
                "multipart_processing_duration_seconds",
                MetricType.HISTOGRAM,
                "Multipart processing duration",
            ),
            (
                "multipart_content_type_mismatches_total",
                MetricType.COUNTER,
                "Total content-type mismatches",
            ),
            # Archive telemetry metrics
            (
                "archive_ratio_max",
                MetricType.GAUGE,
                "Maximum compression ratio detected in archives",
            ),
            (
                "archive_entries_total",
                MetricType.COUNTER,
                "Total archive entries processed",
            ),
            (
                "strict_mode_flag",
                MetricType.GAUGE,
                "Feature flag state for strict mode (0/1)",
            ),
            (
                "strict_mode_flip_total",
                MetricType.COUNTER,
                "Total feature flag flips for annotations",
            ),
            # Redis metrics
            ...
            (
                "redis_circuit_breaker_trips_total",
                MetricType.COUNTER,
            ...
            (
                ...
                MetricType.GAUGE,
                "Security middleware health status",
            ),
            (
                "active_security_contexts",
                MetricType.GAUGE,
...
def record_redaction_failure(error_type: str, tier: int = 1):
    ...
    _metrics_collector.increment_counter(
        "redaction_failures_total", labels={"error_type": error_type, "tier": str(tier)}
    )
...
def record_multipart_rejection(
    ...
    _metrics_collector.increment_counter(
        "multipart_reject_total",
        labels={"reason": reason, "endpoint": endpoint, "tenant": tenant},
    )
