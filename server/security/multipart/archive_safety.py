...
def validate_archive_safety(
    content: bytes, content_type: str, limits: ArchiveLimits | None = None
) -> dict[str, Any]:
    """
    ...
    if content.startswith(b"PK\x03\x04") or content.startswith(b"PK\x05\x06"):
        ...
        try:
            ...
            result["metrics"]["compression_ratio"] = (
                len(content) / len(decompressed) if decompressed else 0
            )
...
def _validate_zip_safety(
    content: bytes, limits: ArchiveLimits, result: dict[str, Any]
) -> dict[str, Any]:
    ...
    try:
        with zipfile.ZipFile(io.BytesIO(content), "r") as zf:
            ...
            result["metrics"]["compression_ratio"] = (
                len(content) / total_uncompressed if total_uncompressed > 0 else 0
            )
