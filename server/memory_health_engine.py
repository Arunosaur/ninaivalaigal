...
class MemoryHealthEngine:
    ...
    async def detect_orphaned_tokens(self, user_id: int) -> List[OrphanedToken]:
        ...
        try:
            ...
            logger.error(
                "Failed to detect orphaned tokens",
                user_id=user_id,
                error=str(e)
            )
            return []
    ...
    async def generate_health_report(self, user_id: int) -> SystemHealthReport:
        ...
        try:
            ...
            system_health = self._determine_system_health(
                healthy_count, warning_count, critical_count, orphaned_count, total_memories
            )
    ...
    async def _get_memory_relevance_score(self, user_id: int, memory_id: str) -> Optional[float]:
    ...
    async def _get_memory_feedback_data(self, user_id: int, memory_id: str) -> Optional[Dict]:
    ...
    async def _check_if_orphaned(self, user_id: int, memory_id: str) -> Optional[OrphanedToken]:
...
_health_engine: Optional[MemoryHealthEngine] = None
