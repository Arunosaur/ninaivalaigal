...
SPEC-065: Encryption at Rest Implementation
- Database encryption: PostgreSQL with pgcrypto extension
- Field-level encryption: Fernet (AES-128) for sensitive fields
- Key management: Environment variables (NINAIVALAIGAL_ENCRYPTION_KEY)
"""
...
if BaseSettings is not None:
    ...
    class StorageConfig(BaseSettings):
        ...
        type: str = Field(default="postgresql", description="Storage type")
        url: str = Field(default_factory=lambda: DEFAULT_RUST_DATABASE_URL, description="Storage URL")
...
# RuntimeConfig - upgraded to Pydantic if available
if BaseSettings is not None:
    from pydantic import Field

    class RuntimeConfig(BaseSettings):
        """Runtime configuration with Pydantic validation."""

        database_url: str = Field(
            default_factory=lambda: DEFAULT_RUST_DATABASE_URL, description="Database connection URL"
        )
        graph_name: str = Field(default="ninaivalaigal_graph", description="Graph database name")
        db_name: str = Field(default="ninaivalaigal_dev", description="PostgreSQL database name")
        service_port: int = Field(default=13398, ge=1, le=65535, description="Service port number")
        service_name: str = Field(default="graphops", description="Service name")
...
def get_config() -> RuntimeConfig:
    ...
    if hasattr(base, "database_url"):
        # Pydantic model
        database_url = base.database_url or os.getenv("DATABASE_URL") or DEFAULT_RUST_DATABASE_URL
    else:
        # Dict fallback
        database_url = base.get("database_url") if isinstance(base, dict) else DEFAULT_RUST_DATABASE_URL
        database_url = database_url or os.getenv("DATABASE_URL") or DEFAULT_RUST_DATABASE_URL
