...
class SuggestionRequest:
    ...
    user_id: int
    memory_id: Optional[str] = None  # Base memory for similarity
    query: Optional[str] = None  # Query-based suggestions
...
class IntelligentSuggestionsEngine:
    ...
    async def _get_high_feedback_memories(
        self, 
        user_id: int
    ) -> List[Dict[str, Any]]:
        """Get memories with high positive feedback"""
    ...
    async def _get_cached_suggestions(
        ...
        try:
            ...
            if cached_data:
                ...
                suggestions = [
                    MemorySuggestion(**s) for s in data["suggestions"]
                ]
                response = SuggestionResponse(
...
_suggestions_engine: Optional[IntelligentSuggestionsEngine] = None
