...
class HTTPSafetySpec:
    ...
    SAFETY_RULES = {
        "content_length_accuracy": {
            ...
            "severity": "critical",
            "fixes": ["Recalculate Content-Length", "Use chunked encoding", "Remove Content-Length header"]
        },
        "response_completeness": {
            "description": "Response body must be complete and not truncated",
            "severity": "critical", 
            "fixes": ["Ensure full response generation", "Add response validation", "Use streaming carefully"]
        },
        "json_serialization": {
            "description": "JSON responses must be properly serialized",
            "severity": "high",
            "fixes": ["Validate JSON before sending", "Handle UUID/datetime serialization", "Use safe serializers"]
        },
        "async_safety": {
            "description": "Async endpoints must not block on synchronous operations",
            "severity": "high",
            "fixes": ["Use async database calls", "Avoid blocking I/O", "Proper async/await usage"]
        },
        "exception_handling": {
            "description": "All exceptions must be caught and return proper HTTP responses",
            "severity": "medium",
            "fixes": ["Global exception handlers", "Endpoint-level try/catch", "Safe error responses"]
        }
    }
