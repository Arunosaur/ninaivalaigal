...
show_stack_status() {
    ...
    $CONTAINER_COMMAND list | grep "$CONTAINER_PREFIX-$NINA_ENV" | awk '{print "  ✓ " $1 " (" $10 ")"}' || echo "  (use '$CONTAINER_COMMAND list' to see all)"
    echo ""
...
main() {
    ...
    log_info "🌍 Environment: ${ENVIRONMENT} (NINA_ENV=${NINA_ENV})"
    log_info "📦 Runtime: ${RUNTIME} (offset: +${RUNTIME_OFFSET:-0})"
    log_info "🏷️  Container Prefix: ${CONTAINER_PREFIX}"
    echo ""
