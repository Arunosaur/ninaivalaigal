...
REDIS_NAME="${REDIS_CONTAINER_NAME:-nv-redis}"
...
c_exists "$REDIS_NAME" && ok "Redis: $REDIS_NAME running" || warn "Redis: $REDIS_NAME not running"
...
port_listen "$REDIS_PORT"&& ok "Redis port $REDIS_PORT open"|| warn "Redis port $REDIS_PORT closed"
...
# Redis health check
if port_listen "$REDIS_PORT"; then
  if container exec nv-redis redis-cli -a nina_redis_dev_password ping >/dev/null 2>&1; then
    ok "Redis: PING OK"
  else
    warn "Redis: PING failed"
  fi
else
  warn "Skipping Redis check (port closed)"
fi
...
echo "  container logs -f ${REDIS_NAME}"
