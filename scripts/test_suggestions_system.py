...
class SuggestionsSystemTester:
    ...
    def test_query_based_suggestions(self) -> bool:
        ...
        try:
            ...
            start_time = time.time()
            response = self.session.post(
                f"{API_BASE_URL}/suggestions/by-query",
                json=payload
            )
            response_time = (time.time() - start_time) * 1000
    ...
    def test_trending_memories(self) -> bool:
        ...
        try:
            ...
            start_time = time.time()
            response = self.session.get(
                f"{API_BASE_URL}/suggestions/trending?limit=10"
            )
            response_time = (time.time() - start_time) * 1000
    ...
    def print_results(self):
        ...
        self.log(f"  ✅ Content-Based Similarity")
        self.log(f"  ✅ Collaborative Filtering")
        self.log(f"  ✅ Feedback-Based Recommendations")
        self.log(f"  ✅ Context-Aware Suggestions")
        self.log(f"  ✅ Personalized Recommendations")
        self.log(f"  ✅ Trending Analysis")
