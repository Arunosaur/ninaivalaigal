...
class RedisPerformanceBenchmark:
        ...
        for user_id in range(users):
            ...
            for strategy, count in strategies.items():
                ...
                for i in range(count):
                    ...
                    ttl = {'recent': 7200, 'frequent': 14400, 'important': 21600, 'context': 3600}[strategy]
    ...
    def benchmark_concurrent_operations(self, concurrent_users: int = 20, operations_per_user: int = 50) -> Dict[str, float]:
        ...
        def user_operations(user_id: int) -> List[float]:
            ...
            for i in range(operations_per_user):
                ...
                if i % 4 == 0:
                    ...
                    score_data = {'score': 0.9, 'updated_at': datetime.utcnow().isoformat()}
    ...
    def cleanup_test_data(self):
        ...
        patterns = ['bench:*', 'memory:user_*', 'relevance:user_*', 'preload:user_*', 'concurrent:*']
        total_deleted = 0
    ...
    def run_full_benchmark(self):
        ...
        print(f"SPEC-033 Memory Retrieval: {memory_actual:.2f}ms (Target: <{memory_retrieval_target}ms) {'✅' if memory_actual < memory_retrieval_target else '❌'}")
        
        # SPEC-031 targets  
        relevance_target = 5  # Target: <5ms
        relevance_actual = self.results['relevance_scoring']['ranking_time_ms']
        print(f"SPEC-031 Relevance Ranking: {relevance_actual:.2f}ms (Target: <{relevance_target}ms) {'✅' if relevance_actual < relevance_target else '❌'}")
        
        # SPEC-038 targets
        preload_target = 30000  # Target: <30 seconds for preloading
        preload_actual = self.results['preloading_simulation']['preload_avg_ms']
        print(f"SPEC-038 Memory Preloading: {preload_actual:.2f}ms per user (Target: <{preload_target}ms) {'✅' if preload_actual < preload_target else '❌'}")
        
        print(f"\n📈 Redis Memory Usage: {initial_info['used_memory_human']} → {final_info['used_memory_human']}")
        print(f"📊 Commands Processed: {final_info['total_commands_processed'] - initial_info['total_commands_processed']}")
