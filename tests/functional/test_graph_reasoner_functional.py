...
from server.graph.models.node_models import create_user_node, create_memory_node, MemoryType
...
async def test_graph_data(age_client):
    ...
    memory_node1 = create_memory_node("test_mem_001", "SPEC-061 Implementation", "Graph reasoner testing", MemoryType.ARCHITECTURE)
    memory_node2 = create_memory_node("test_mem_002", "Redis Integration", "Performance caching layer", MemoryType.CORE)
...
class TestInferRelevanceFunctional:
    ...
    async def test_infer_relevance_finds_connected_memories(self, graph_reasoner, test_graph_data):
        ...
        if inference.suggested_memories:
            ...
            assert (connected_memory in inference.suggested_memories or 
                   inference.proximity_metrics.get(connected_memory, 0) > 0)
...
class TestIntegrationScenarios:
    ...
    async def test_performance_under_load(self, graph_reasoner, test_graph_data):
        ...
        for i, memory_id in enumerate(memory_ids):
            tasks.append(graph_reasoner.feedback_loop(
    ...
    async def test_error_handling_and_recovery(self, graph_reasoner, test_graph_data):
        ...
        try:
            result = await graph_reasoner.feedback_loop(
                user_id, test_graph_data["memory_ids"][0], "relevance", 1.5  # Invalid score > 1.0
            )
