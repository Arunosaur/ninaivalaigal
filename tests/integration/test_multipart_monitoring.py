...
class TestMultipartMonitoring:
    ...
    def test_prometheus_format_export(self):
        ...
        assert (
            "multipart_reject_total{endpoint=/api/upload,reason=archive_blocked,tenant=tenant-1} 1" in prometheus_output
        )
    ...
    def test_alert_threshold_simulation(self):
        ...
        total_rejections = sum(value for key, value in metrics.items() if key.startswith("multipart_reject_total"))
        ...
        archive_rejections = sum(value for key, value in metrics.items() if "reason=archive_blocked" in key)
    ...
    def test_canary_monitoring_scenario(self):
        ...
        archive_blocks = sum(value for key, value in metrics.items() if "reason=archive_blocked" in key)
