...
from server.intelligence.models import (
    GraphContext, ScoredMemory, RelationshipPrediction, EmbeddingUpdate,
    UserFeedback, WeightUpdate, TeamContext, GraphMLMetrics
)
...
class TestGraphMLEngine:
    ...
    async def test_predict_memory_relevance(
        ...
        with patch.object(ml_engine, '_extract_graph_features') as mock_features, \
    ...
    async def test_predict_relationships(self, ml_engine, graph_context):
        ...
        with patch.object(ml_engine, '_extract_pair_features') as mock_features, \
            ...
            mock_predict.side_effect = [
                ...
                {'confidence': 0.5, 'relationship_type': 'weak_connection'},  # Below threshold
    ...
    async def test_optimize_embeddings(self, ml_engine):
        ...
        with patch.object(ml_engine, '_extract_memory_graph_features') as mock_features, \
            ...
            mock_enhance.return_value = [0.15, 0.25, 0.35, 0.45, 0.55]  # Enhanced embedding
    ...
    async def test_adapt_ranking_weights(self, ml_engine, user_feedback_batch):
        ...
        with patch.object(ml_engine, '_analyze_feedback_patterns') as mock_analyze, \
            ...
            weight_update = await ml_engine.adapt_ranking_weights(
                feedback_batch=user_feedback_batch,
                learning_rate=0.01
            )
    ...
    async def test_relationship_training_workflow(self, ml_engine):
        ...
        with patch.object(ml_engine, '_prepare_training_data') as mock_prepare, \
