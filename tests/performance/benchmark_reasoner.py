...
class TestAnalyzeMemoryNetworkPerformance:
    ...
    def test_analyze_network_performance_large_network(self, benchmark, graph_reasoner, mock_age_client):
        ...
        large_network = {
            ...
            "edges": [
                {"source": f"node_{i:03d}", "target": f"node_{(i+1)%100:03d}", "type": "LINKED_TO"}
                for i in range(100)
            ]
        }
...
class TestConcurrentOperationsPerformance:
    ...
    def test_concurrent_mixed_operations_performance(self, benchmark, graph_reasoner, mock_age_client, mock_redis_client):
        """Benchmark mixed concurrent operations"""
        # Setup responses for all operation types
        mock_age_client.execute_cypher.side_effect = [
            # explain_context responses
            [{'path': {'nodes': [{'id': 'user_001'}, {'id': 'mem_001'}], 'relationships': [{'type': 'CREATED'}]}, 'weights': [0.9], 'depth': 1}],
            [{'related.id': 'macro_001', 'timestamp': '2024-09-21T20:00:00Z', 'rel_type': 'LINKED_TO'}],
            # infer_relevance responses
            [{"memory_id": "mem_002", "distance": 1}],
            [{"agent_id": "agent_001"}],
            [{"distance": 1}], [{"distance": 2}],  # proximity
    ...
    def test_high_concurrency_performance(self, benchmark, graph_reasoner, mock_age_client):
        ...
        mock_age_client.execute_cypher.side_effect = [
            [{'path': {'nodes': [{'id': 'user_001'}, {'id': f'mem_{i:03d}'}], 'relationships': [{'type': 'CREATED'}]}, 'weights': [0.9], 'depth': 1}]
            for i in range(20)
        ] + [
            [{'related.id': 'macro_001', 'timestamp': '2024-09-21T20:00:00Z', 'rel_type': 'LINKED_TO'}]
            for _ in range(20)
...
class TestPerformanceSLOs:
    ...
    def test_infer_relevance_slo_compliance(self, graph_reasoner, mock_age_client):
        ...
        mock_age_client.execute_cypher.side_effect = [
            ...
            [{"distance": 1}], [{"distance": 1}],  # Proximity
            [{"base_score": 0.8, "connection_count": 5}],  # Memory scoring
            [{"activity_count": 10, "capabilities": "test"}]  # Agent scoring
        ]
