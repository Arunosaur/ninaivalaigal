...
class TestStatusCodeIntegration:
    ...
    async def test_413_part_count_limits(self):
        ...
        parts = [
            FakePart({"content-type": "text/plain"}, [b"test"]) for _ in range(300)
        ]
        req = FakeRequest(parts)
    ...
    async def test_415_binary_masquerade(self):
        ...
        parts = [
            FakePart({"content-type": "text/plain"}, [pe_magic + b"fake_pe_content"])
        ]
        req = FakeRequest(parts)
        ...
        assert (
            "utf" in ei.value.detail.lower()
            or "binary content detected" in ei.value.detail.lower()
        )
    ...
    async def test_415_archive_blocked(self):
        ...
        parts = [
            FakePart({"content-type": "text/plain"}, [zip_magic + b"fake_zip_content"])
        ]
        req = FakeRequest(parts)
        ...
        assert ei.value.status_code == 415
        assert (
            "archive" in ei.value.detail.lower()
            or "binary content detected" in ei.value.detail.lower()
        )
    ...
    async def test_415_binary_on_text_endpoint(self):
        ...
        parts = [
            FakePart({"content-type": "application/octet-stream"}, [b"binary_data"])
        ]
        req = FakeRequest(parts)
