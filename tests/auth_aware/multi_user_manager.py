...
from .models import (
    TestUser, TestSession, AuthTestResults, ConflictResults, 
    IsolationResults, UserRole, TestUserStatus, AuthTestResult
)
...
class MultiUserTestManager:
    ...
    async def test_session_conflicts(
        self, 
        users: List[TestUser]
    ) -> ConflictResults:
        """
    ...
    async def _test_cross_team_admin_access(
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                access_blocked = (
                    response.status_code in [403, 404] or
                    user.role not in [UserRole.ADMIN]
                )
