...
class TestSecurityScenarios:
    ...
    async def test_injection_attack_prevention(
        ...
        for result in injection_results:
            assert result.is_secure
            assert result.response_code in [
                400,
                401,
                403,
                422,
            ]  # Bad request or blocked
    ...
    async def test_soc2_compliance_validation(
        ...
        compliance_results = await security_engine.validate_compliance_requirements(
            compliance_standard="SOC2", test_scenarios=soc2_scenarios
        )
    ...
    async def test_gdpr_compliance_validation(
        ...
        compliance_results = await security_engine.validate_compliance_requirements(
            compliance_standard="GDPR", test_scenarios=gdpr_scenarios
        )
    ...
    async def _test_xss_payload(
        self, security_engine: SecurityScenarioEngine, user, token: str, payload: str
    ) -> SecurityTestResult:
        """Test XSS payload"""
