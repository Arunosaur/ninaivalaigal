...
from .models import UserRole, PermissionTestResult, AuthTestResult
...
class TestRBACValidation:
    ...
    async def test_admin_role_permissions(
        ...
        admin_endpoints = [result for result in permission_results 
                          if result.expected_result == "allow"]
    ...
    async def test_member_role_permissions(
        ...
        forbidden_endpoints = [r.endpoint for r in forbidden_results]
        assert any('/admin/' in endpoint for endpoint in forbidden_endpoints)
        assert any('DELETE' in r.method for r in forbidden_results 
                  if 'memories' in r.endpoint)
    ...
    async def test_permission_boundary_enforcement(
        ...
        for user in all_role_users:
            ...
            escalation_attempts = [r for r in boundary_results 
    ...
    async def test_role_permission_matrix_validation(
        ...
        admin_allowed = len([r for r in role_results[UserRole.ADMIN] 
                           if r.expected_result == "allow"])
        lead_allowed = len([r for r in role_results[UserRole.TEAM_LEAD] 
                          if r.expected_result == "allow"])
        member_allowed = len([r for r in role_results[UserRole.MEMBER] 
                            if r.expected_result == "allow"])
        viewer_allowed = len([r for r in role_results[UserRole.VIEWER] 
                            if r.expected_result == "allow"])
        guest_allowed = len([r for r in role_results[UserRole.GUEST] 
                           if r.expected_result == "allow"])
    ...
    async def test_concurrent_rbac_validation(
        ...
        successful_tests = sum(1 for result in concurrent_results 
