...
class TestScopeHierarchy:
    ...
    async def test_conflict_resolution_overlapping_scopes(self, mock_scope_manager):
        ...
        for scenario in overlapping_scenarios:
            ...
            permission_hierarchy = ["view", "comment", "edit", "share", "admin", "delete"]
            if valid_permissions:
                highest_permission = max(valid_permissions, key=lambda x: permission_hierarchy.index(x) if x in permission_hierarchy else -1)
    ...
    async def test_scope_validation_edge_cases(self, mock_scope_manager):
        ...
        for case in edge_cases:
            if case["case"] == "empty_scope":
                ...
                if len(parts) == 3:
                    assert parts != ["user", "team", "org"], "Hierarchy order should be org/team/user"
            elif case["case"] == "valid_nested_scope":
                parts = case["scope"].split("/")
                assert parts == ["organization", "team", "user"], "Valid hierarchy should be org/team/user"
