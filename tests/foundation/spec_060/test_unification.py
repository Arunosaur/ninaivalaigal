...
class TestUnification:
    ...
    def test_unified_caching_strategy(self, mock_graph_client):
        ...
        cache_scenarios = [
            ...
            },
            {
                ...
                "invalidation_triggers": [
                    "memory_added_to_context",
                    "memory_removed_from_context",
                ],
            },
        ]
        ...
        for scenario in cache_scenarios:
            ...
            assert (
                len(scenario["invalidation_triggers"]) > 0
            ), f"Cache scenario {scenario['cache_type']} should have invalidation triggers"
    ...
    def test_unified_integration_performance(self, mock_graph_client):
        ...
        for step in workflow_steps:
            ...
            assert (
                step_duration <= step["target_ms"] * 2
            ), f"Step {step['step']} took {step_duration:.2f}ms, target was {step['target_ms']}ms"

        # Validate total workflow performance
        target_total_ms = sum(step["target_ms"] for step in workflow_steps)
        assert (
            total_time <= target_total_ms * 1.5
        ), f"Total workflow took {total_time:.2f}ms, target was {target_total_ms}ms"
