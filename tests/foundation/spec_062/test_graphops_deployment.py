...
class TestGraphOpsDeployment:
    ...
    def mock_container_client(self):
        ...
        client.list_containers.return_value = [
            {
                "name": "ninaivalaigal-graph-db",
                "status": "running",
                "ports": ["5433:5432"],
            },
            {
                "name": "ninaivalaigal-graph-redis",
                "status": "running",
                "ports": ["6380:6379"],
            },
        ]
    ...
    def test_dual_architecture_support(self, deployment_config):
        ...
        for arch in architectures:
            ...
            assert container_runtime in [
                "container",
                "docker",
            ], f"Should use appropriate runtime for {arch}"
    ...
    def test_container_orchestration(self, mock_container_client, deployment_config):
        ...
        for operation in lifecycle_operations:
            ...
            if "startup_order" in operation:
                ...
                ), "Startup order should match container count"
    ...
    def test_network_configuration(self, deployment_config):
        ...
        graph_ports = [
            network_config["graph_db_port"],
            network_config["graph_redis_port"],
        ]
    ...
    def test_security_configuration(self, deployment_config):
        ...
        for config in security_config:
            assert (
                len(config["security_measures"]) > 0
            ), f"Component {config['component']} should have security measures"
            assert (
                config["access_controls"]["external_access"] == "localhost_only"
            ), f"Component {config['component']} should restrict external access"
    ...
    def test_backup_and_recovery(self):
        ...
        for strategy in backup_strategies:
            assert strategy["backup_frequency"] in [
                "hourly",
                "daily",
                "weekly",
