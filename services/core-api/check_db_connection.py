...
os.environ["NINA_DB_PASSWORD"] = "dev_password_change_in_production"  # pragma: allowlist secret
...
try:
    ...
    session = db.get_session()
    result = session.execute(text("SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'users'"))
