...
async def analyze_injection_opportunities(
    ...
    try:
        ...
        candidates = await injection_engine.analyze_injection_opportunities(
            context=context, max_candidates=request.max_candidates
        )
        ...
        logger.error(
            "Failed to analyze injection opportunities",
            error=str(e),
...
async def get_injection_analytics(
    ...
    try:
        ...
        logger.error(
            "Failed to get injection analytics",
            error=str(e),
            user_id=current_user.get("user_id"),
        )
