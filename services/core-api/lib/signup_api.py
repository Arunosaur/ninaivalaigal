...
class SignupRequest(BaseModel):
    ...
    account_type: str = Field(default="individual", max_length=64)
    accept_terms: Optional[bool] = Field(default=True)
...
async def signup_user(request: SignupRequest, background_tasks: BackgroundTasks) -> SignupResponse:
    """Primary signup endpoint returning JWT on success."""

    if request.accept_terms is False:
        raise HTTPException(status_code=400, detail="You must accept the terms to create an account")

    signup_payload = IndividualUserSignup(
        email=request.email,
    ...
    result = create_individual_user(signup_payload)
