...
async def register_provider(
    ...
    try:
        ...
        if config.provider_type is not ProviderType.POSTGRES:
            raise HTTPException(status_code=400, detail="Only PostgreSQL providers are supported")

        await registry.register_provider(config, PostgresMemoryProvider)
