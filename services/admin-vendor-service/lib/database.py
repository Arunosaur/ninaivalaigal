...
#
"""Database models and operations for the admin vendor service."""
...
class DatabaseManager:
    ...
    def __init__(self, config=DEFAULT_RUST_DATABASE_URL):
        """Initialize instance."""
        ...
        if isinstance(database_url, str) and database_url.startswith("sqlite"):
            ...
            if not str(database_url).startswith("postgresql"):
                raise ValueError(
                    f"Invalid database URL format. Expected postgresql:// URL, got: {str(database_url)[:50]}..."
                )
            resolved_url = database_url
