...
from lib.auth import get_current_user, get_db

# Import from lib - main.py sets up path so lib is available
from lib.database import Memory, Team, User
...
class GraphIntelligenceEngine:
    ...
    async def enhance_context_with_graph(
        self,
        memory_id: str,
        context_type: str,
        ...
        overall_confidence = (
            sum(related_entities["confidence_scores"].values()) / len(related_entities["confidence_scores"])
...
async def graph_integration_health():
    ...
    try:
        ...
        if hasattr(redis_client, "redis") and hasattr(redis_client.redis, "ping"):
