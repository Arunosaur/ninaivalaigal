...
class MemoryProviderRegistry:
        ...
        try:
            ...
            for provider_data in config_data.get("providers", []):
                try:
                    ...
                    if provider_type and provider_type.lower().startswith("mem0"):
                        logger.warning(
                            "Skipping legacy mem0 provider entry %s; mem0 support has been removed",
    ...
    async def _auto_discover_providers(self) -> None:
        ...
        try:
            ...
            if os.getenv("MEM0_URL") or os.getenv("ENABLE_LEGACY_MEM0"):
                logger.warning("Legacy mem0 settings detected but mem0 providers are no longer supported; ignoring")
    ...
    async def _activate_provider(self, name: str) -> bool:
        ...
        try:
            ...
            if registered.config.provider_type == ProviderType.POSTGRES:
                ...
                raise ValueError(
                    f"Unsupported provider type: {registered.config.provider_type}. Only PostgreSQL providers are supported."
                )
